 ...
 */
public extension SegueHandler where Self: ViewController, SegueIdentifier.RawValue == String {
    /**
     ...
     */
    func segueIdentifier(for segue: StoryboardSegue) -> SegueIdentifier {
        /*
    ...
    func segueIdentifier(for identifier: String) -> SegueIdentifier {
        /*
        Map the identifier string to an enum. It's a programmer error
        if a segue identifier string that's provided doesn't map to one of the
        raw representable values (most likely enum cases).
        */
